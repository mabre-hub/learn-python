...
def fourier_approx(x):
    s = a_val # if we's use a0() here not only the definition but also a0() would get calculated multiple times but is only needed once
    for n in range (1, N+1):                    # loops from 1 no N
        s += A[n-1] * np.sin( omega(n) * x ) + B[n-1] * np.cos( omega(n) * x)  # equation 1
    return s

x_plot = np.linspace ( 0, L, 1000)  # creates 1000 evenly spaced values between 0 and L

plt.plot( x_plot, f(x_plot), label = "Original f(x)", color = "blue")                               # plots f(x) > the original function, for all 1000 x_plot values
plt.plot( x_plot, fourier_approx(x_plot), "--", label = f"Fourier Approx (N={N})", color = "red")  # plots the fourrier approximation as a dashed line
plt.xlabel("x")     # labels the x axis
plt.ylabel("f(x)")  # labels the y axis
plt.legend()        # plots a leged using the label terms from above
plt.grid(True)      # plots ggrid
plt.title("Part 1: Fourier Coefficients for an Analytical Function", weight = "bold")             # sets title of the plot
